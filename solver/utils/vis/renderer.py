...
class Renderer:
    def __init__(self, width, height, focal_length=None, device="cuda", faces=None, K=None, bin_size=None,faces_human=None,faces_obj=None):
        ...
        self.faces_human=torch.from_numpy((faces_human).astype("int")).unsqueeze(0).to(self.device)
        self.faces_obj=torch.from_numpy((faces_obj).astype("int")).unsqueeze(0).to(self.device)
    ...
    def render_mesh_hoi(self, human_vertices, object_vertices, background=None, human_color=[0.8, 0.8, 0.8], object_color=[0.3, 0.5, 0.7], VI=50):
        """
        Render human and object meshes together with different colors.
        :param human_vertices: Vertices of the human mesh.
        :param object_vertices: Vertices of the object mesh.
        :param background: Background image.
        :param human_color: Color for the human mesh.
        :param object_color: Color for the object mesh.
        :param VI: Vertex interval for bounding box computation.
        """
        # Combine human and object vertices for bounding box update
        combined_vertices = torch.cat((human_vertices[::VI], object_vertices[::VI]), dim=0)
        self.update_bbox(combined_vertices, scale=1.2)

        # Debugging prints
        # print("Human vertices shape:", human_vertices.shape)
        # print("Object vertices shape:", object_vertices.shape)
        # print("Human faces shape:", self.faces_human.shape)
        # print("Object faces shape:", self.faces_obj.shape)

        # Validate face indices
        assert torch.max(self.faces_human) < human_vertices.shape[0], "Invalid face indices in human mesh."
        assert torch.max(self.faces_obj) < object_vertices.shape[0], "Invalid face indices in object mesh."

        # Prepare human mesh
        human_vertices = human_vertices.unsqueeze(0)
        human_features = torch.tensor(human_color).reshape(1, 1, 3).to(device=human_vertices.device, dtype=human_vertices.dtype)
        human_features = human_features.repeat(1, human_vertices.shape[1], 1)
        # print('feature, human_features.shape:', human_features.shape)
        human_textures = TexturesVertex(verts_features=human_features)
        human_mesh = Meshes(verts=human_vertices, faces=self.faces_human, textures=human_textures)

        # Prepare object mesh
        object_vertices = object_vertices.unsqueeze(0)
        # object_features = torch.tensor(object_color).reshape(1, 1, 3).to(device=object_vertices.device, dtype=object_vertices.dtype)
        # object_features = object_features.repeat(1, object_vertices.shape[1], 1)
        # Broadcast object_color to per-vertex if it is a single RGB value
        obj_col_tensor = torch.as_tensor(object_color, device=object_vertices.device, dtype=object_vertices.dtype)
        if obj_col_tensor.ndim == 1 and obj_col_tensor.numel() == 3:
            # single color -> (1, V, 3)
            object_features = obj_col_tensor.view(1, 1, 3).repeat(1, object_vertices.shape[1], 1)
        elif obj_col_tensor.ndim == 2 and obj_col_tensor.shape[-1] == 3:
            # per-vertex colors (V,3) -> (1, V, 3)
            object_features = obj_col_tensor.unsqueeze(0)
        else:
            # fallback: try to reshape to (1, V, 3)
            object_features = obj_col_tensor.view(1, object_vertices.shape[1], 3)
        object_textures = TexturesVertex(verts_features=object_features)
        object_mesh = Meshes(verts=object_vertices, faces=self.faces_obj, textures=object_textures)

        # Combine meshes into a scene
        combined_mesh = join_meshes_as_scene([human_mesh, object_mesh])

        # Render the scene
        results = torch.flip(self.renderer(combined_mesh, cameras=self.cameras, lights=self.lights), [1, 2])
        image = results[0, ..., :3] * 255
        mask = results[0, ..., -1] > 1e-3
    ...
    def render_with_ground(self, verts, colors, cameras, lights, faces=None):
        ...
        N, V, _ = verts.shape
        if faces is None:
            faces = self.faces_human.clone().expand(N, -1, -1)
        else:
    ...
    def render_with_ground_hoi(self, human_vertices, object_vertices, cameras, lights, human_color=[0.8, 0.8, 0.8], object_color=[0.3, 0.5, 0.7]):
        """
        Render human and object meshes together with ground.
        :param human_vertices: Vertices of the human mesh (V, 3)
        :param object_vertices: Vertices of the object mesh (V, 3)
        :param cameras: Camera parameters
        :param lights: Light parameters
        :param human_color: Color for the human mesh
        :param object_color: Color for the object mesh
        """
        # Prepare human mesh
        ...
        # Prepare object mesh
        object_vertices = object_vertices.unsqueeze(0)
        object_features = torch.tensor(object_color).unsqueeze(0).to(device=object_vertices.device, dtype=object_vertices.dtype)
        object_textures = TexturesVertex(verts_features=object_features)
        object_mesh = Meshes(verts=object_vertices, faces=self.faces_obj, textures=object_textures)

        # Prepare ground mesh
        gv, gf, gc = self.ground_geometry
        ground_textures = TexturesVertex(verts_features=gc[..., :3].unsqueeze(0))
        ground_mesh = Meshes(verts=gv.unsqueeze(0), faces=gf.unsqueeze(0), textures=ground_textures)

        # Combine all meshes into a scene
        combined_mesh = join_meshes_as_scene([human_mesh, object_mesh, ground_mesh])

        materials = Materials(device=self.device, shininess=0)

        results = self.renderer(combined_mesh, cameras=cameras, lights=lights, materials=materials)
        image = (results[0, ..., :3].cpu().numpy() * 255).astype(np.uint8)
